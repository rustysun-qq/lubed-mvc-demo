...
<?php if(isset($name)): ?>
    <?php foreach ($name as $i => $nameSection): ?>
      <?php if ($i == count($name) - 1): ?>
        <span class="exc-title-primary"><?php echo $view->escape($nameSection) ?></span>
      <?php else: ?>
        <?php echo $view->escape($nameSection) . ' \\' ?>
      <?php endif ?>
    <?php endforeach ?>
<?php endif;?>
    <?php if (isset($code)&&$code): ?>
      <span title="Exception Code">(<?php echo $view->escape($code) ?>)</span>
    <?php endif ?>
  </div>

  <div class="exc-message">
    <?php if (!empty($message)): ?>
      <span><?php echo $view->escape($message) ?></span>
      <?php if (!empty($previousMessages)): ?>
        ...
        <ul>
          <?php foreach ($previousMessages as $i => $previousMessage): ?>
            <li>
              <?php echo $view->escape($previousMessage) ?>
    ...
    <span id="plain-exception">
        <?php echo isset($plain_exception)?$plain_exception->result:''; ?>
    </span>
    <button id="copy-button" class="rightButton clipboard" data-clipboard-text="<?php echo isset($plain_exception)?$view->escape($plain_exception->result):''; ?>" title="Copy exception details to clipboard">
      COPY
    </button>
